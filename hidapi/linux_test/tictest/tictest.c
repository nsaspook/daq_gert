...
bool hidrawapi_mcp2210_init(void) {
    ...
    memset(buf, 0, 64); // buf initialized to zeros
    buf[0] = 0x30; // command 30 - set GPIO pin's level to all high
    buf[4] = 0xff;
    buf[5] = 0xff;

    res = hid_write(handle, buf, 64); // write setting into MCP2210
    if (res < 0) {
        printf("Unable to write()\n");
...
void setup_tic12400_transfer(void) {
    ...
    buf[10] = 0x00; // set CS active values to 0
    buf[11] = 0x00;
    buf[18] = 0x04; // set no of bytes to transfer = 4 // 32-bit transfers
...
void get_tic12400_transfer(void) {
    // ---------- Get SPI transfer settings (0x41)-------------
    buf[0] = 0x41; // 0x41 Get SPI transfer settings
    res = hid_write(handle, buf, 2); // read setting from MCP2210
    if (res < 0) {
        printf("Unable to write()\n");
        printf("Error: Get SPI transfer settings %ls\n", hid_error(handle));
    }
